...
func _on_logged_in(status: EGClient.LOGIN_STATUS):
	...
	logger.info("Logged in. Updating library cache from GOG.")
...
# Return a list of installed gog apps. Optionally caching flags can be passed to
...
func _load_library(
	...
	logger.info("Fetching GOG library...")
	...
	for app_id in app_info.keys():
		...
		item.command = "gog"
		item.args = ["-gamepadui", "-gogos3", "-gogpal", "-gogdeck", "-silent", "gog://rungameid/" + item.provider_app_id]
		item.tags = ["gog"]
	...
	if caching_flags & Cache.FLAGS.SAVE:
		...
		if Cache.save_json(_cache_dir, _apps_cache_file, json_items) != OK:
			logger.warn("Unable to save GOG apps cache")
